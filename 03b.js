...
const findTripledLetter = (rucksacks) => {
  const firstRucksackLength = rucksacks[0].length

  for (let i = 0; i < firstRucksackLength; i++) {
    const letter = rucksacks[0][i]

    if (rucksacks[1].includes(letter) && rucksacks[2].includes(letter)) return letter
  }

  throw new Error('Too messy rucksacks!')
}
...
while (i < input.length) {
  const rucksacksGroup = [input[i], input[i + 1], input[i + 2]]
  const tripledLetter = findTripledLetter(rucksacksGroup)

  score += rateLetter(tripledLetter)
  i += 3
}
